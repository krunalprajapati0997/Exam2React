...
const Material = () => {
    ...
    const handleClick = () => {
        ...
        console.log(item)
        axios.post("http://localhost:8009", item).then((res) => {
        ...
        switch (name) {
            ...
            case 'mobilenumber':
                if (!new RegExp(/^((\+)?(\d{2}[-]))?/).test(value)) {
                    setErrors({
                        ...errors,
                        mobilenumber: 'mobilenumber atleast have 10'
                    })
                } else {
            ...
            <Grid>
                <Paper elevation={20} style={paperStyle}>
                    ...
                    <TextField name='name' fullWidth label='Name' value={values.name} onChange={handleChange} error={Boolean(errors.name)} helperText={errors.name} />
                        ...
                        <TextField name='mobilenumber' fullWidth label='mobilenumber' value={values.mobilenumber} onChange={handleChange} error={Boolean(errors.mobilenumber)} helperText={errors.mobilenumber} />
